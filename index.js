...
async function translateFile(options) {
  ...
  if (translateReq.status !== 200) {
    const errorMapping = {
      400: "The body of the request is not a valid JSON.",
      402: "You don't have an active subscription. Please create a subscription to use the service.",
    };
    console.error(
      errorMapping[translateReq.status] ||
